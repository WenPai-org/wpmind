...
class AudioService extends AbstractService {
	...
	public function transcribe( string $audio_file, array $options = [] ) {
		...
		return $this->execute_with_failover(
			...
			$provider,
			$context,
			function ( string $try_provider, array $endpoint ) use ( $file_content, $options, $audio_file ) {
				...
				if ( $status_code !== 200 ) {
					...
					return new WP_Error(
						'wpmind_transcribe_error',
						sprintf( __( '转录 API 错误 (%1$d): %2$s', 'wpmind' ), $status_code, $error_message )
					);
				}
	...
	public function speech( string $text, array $options = [] ) {
		...
		return $this->execute_with_failover(
			...
			$provider,
			$context,
			function ( string $try_provider, array $endpoint ) use ( $text, $options ) {
				$api_key  = $endpoint['api_key'];
				$base_url = $endpoint['custom_base_url'] ?? $endpoint['base_url'] ?? '';
				...
				if ( $status_code !== 200 ) {
					...
					return new WP_Error(
						'wpmind_speech_error',
						sprintf( __( '语音合成 API 错误 (%1$d): %2$s', 'wpmind' ), $status_code, $error_message )
					);
				}
